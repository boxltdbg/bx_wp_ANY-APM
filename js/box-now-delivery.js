(function ($) {
  /**
   * Check if Box Now Delivery shipping method is selected.
   */
  function isBoxNowDeliverySelected() {
    var selectedShippingMethod = $('input[name^="shipping_method"]:checked').val();
    return selectedShippingMethod && selectedShippingMethod.indexOf('box_now_delivery') === 0;
  }

  /**
   * Get the label for the Box Now Delivery shipping method.
   */
  function getBoxNowDeliveryLabel() {
    var shippingMethods = $('input[name^="shipping_method"]');
    var label = null;
    shippingMethods.each(function () {
      var $this = $(this);
      var value = $this.val();
      if (value && value.indexOf('box_now_delivery') === 0) {
   ...
   */
  function addButton() {
    var label = getBoxNowDeliveryLabel();
    if (label && $("#box_now_delivery_button").length === 0 && boxNowDeliverySettings.displayMode === "popup") {
    ...
    } else if (label && boxNowDeliverySettings.displayMode === "embedded") {
      label.after(
  ...
  /**
   * Embed the map to the WooCommerce shipping fields.
   */
  function embedMap() {
    ...
    // Move the map to the WooCommerce shipping fields when the BoxNow delivery option is selected
  ...
  $(document).ready(function () {
    ...
    function addOrderValidation() {
      $(document.body).on("click", "#place_order", function (event) {
        ...
        if (!lockerData && isBoxNowDeliverySelected()) {
          event.preventDefault();
